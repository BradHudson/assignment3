...
def evaluate(clf, training_x, testing_x, training_y, testing_y, title=""):
    ...
    plot_learning_curve(train_scores, test_scores, train_sizes, 'part5plots/' + title, title= "Learning Curve for " + title)
...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(np.array(knn_X), np.array(wine_Y), test_size=0.30)
    ...
    X_train, X_test, y_train, y_test = train_test_split(np.array(em_X), np.array(wine_Y), test_size=0.30)
    learner = MLPClassifier(hidden_layer_sizes=(22,), activation='relu', learning_rate_init=0.0051,
                            random_state=seed)
